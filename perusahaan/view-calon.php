                        ...
                        <div class="panel-body">
                            <div class="table-responsive">
                                <table class="table table-striped table-bordered table-hover" id="dataTables-example">
                                    ...
                                    <?php 
                                    $id = $_GET['id'];
                                    $query = mysqli_query($con, "select * from datacurriculumvitae where lowongan_id='$id'");
                                    $no = 1;
                                ...
                                <?php 
                                    $id = $_GET['id'];
                                    $query7 = mysqli_query($con, "select count(statuslamaran) from datacurriculumvitae where statuslamaran='diterima' and lowongan_id='$id'");
                                    $data7 = mysqli_fetch_array($query7)

                                    ?>
                                    <?php 
                                    $id = $_GET['id'];
                                    $query8 = mysqli_query($con, "select * from datalowongan where lowongan_id='$id' ");
                                    $data8 = mysqli_fetch_array($query8);

                                    if($data8['jumlahorangyangdiperlukan']<=$data7['count(statuslamaran)']){
                                            $id = $_GET['id'];
                                            mysqli_query($con, "update datalowongan set statuslowonganpekerjaan='terisi' where lowongan_id='$id'");

                                    } 
                                    else{
                                            $id = $_GET['id'];
                                            mysqli_query($con, "update datalowongan set statuslowonganpekerjaan='belum terisi' where lowongan_id='$id'");
